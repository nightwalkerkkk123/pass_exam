...
**ASR** 把音频波形转成文字。Whisper、SenseVoice 等模型转录 5 秒音频，在 GPU 上部署中小规格模型时通常需要 50-200ms；更大规格的模型或资源受限的部署环境则会达到 200-500ms（实验 9-4 中的对照组即属后者）。更关键的问题在于：在整个 VAD 等待与 ASR 转录过程中，后面的 LLM 完全闲着，没收到任何信息，无法提前开始思考。
...
> **实验 9-4 ★：使用 Qwen2-Audio 模拟流式语音感知**
>
...
> **实验 9-3 ★★★：使用 Step-Audio R1 实现端到端语音思考**
>
...
>
> 使用 Fish Audio S1 的声音克隆能力（只需 3-10 秒参考语音就能零样本克隆出同一音色）。构建 12 条参考语音库，覆盖情绪（中性/高兴/沮丧/思考）x 语速（正常/快/慢）x 风格（正式/轻松），每条约 5 秒。
>
